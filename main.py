...
def displayCategory():
    ...
    with sqlite3.connect('database.db') as conn:
        ...
        data = cur.fetchall()
        if not data:
            cur.execute("SELECT name FROM categories WHERE categoryId = ?", (category_id,))
    ...
    if request.method == "POST":
        ...
        with sqlite3.connect('database.db') as conn:
            ...
            if (password == old_password):
                try:
                    cur.execute("UPDATE users SET password = ? WHERE userId = ?", (new_password, userId))
                    conn.commit()
...
def add_to_cart():
    if 'email' not in session:
        ...
        with sqlite3.connect('database.db') as conn:
            ...
            try:
                cur.execute("INSERT INTO kart (userId, productId) VALUES (?, ?)", (userId, product_id))
                conn.commit()
                msg = "Added successfully"
            except:
                conn.rollback()
